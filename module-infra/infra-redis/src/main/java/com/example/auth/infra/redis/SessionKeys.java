 ...
 * Cluster 슬롯에 놓인다 — 회전 Lua가 KEYS와 내부 DEL 루프로 만지는 모든 키의 원자성을 Cluster에서도
 * 보존한다. {@code refidx}는 {@code jti→userId}를 resolve해야 해 태그를 담을 수 없으므로 의도적으로 슬롯
 * 밖이며 Lua 원자 경계에 넣지 않는다(단일 키 GET/SET).
 */
