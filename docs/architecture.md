...
- 도메인마다 PostgreSQL 스키마 하나를 소유한다.
  ...
  - 도메인 의미 없는 기술 테이블(메시징 디둡·DLQ)은 도메인 스키마에 두지 않는다. 구현 infra 모듈이 전용 스키마(`msg` — infra-messaging 소유)와 그 마이그레이션을 소유하고 같은 방식으로 등록한다.
    - 특정 도메인 스키마에 얹으면 여러 도메인·앱이 소비하는 데이터가 한 도메인에 귀속돼 경계가 오염된다. 기술 스키마는 배포 단위마다 자기 DB에 서므로 물리 분리 시 그대로 따라간다.
...
- 예외: 온보딩 가입 완료(`CreateUser`)는 usr·auth 두 스키마를 한 ACID 트랜잭션으로 원자 변경한다(단일 PostgreSQL + 스키마 분리 토폴로지 전제).
  - 사가·이벤트 분리는 고아-User 전진복구·재조정 스윕 비용으로 검토 후 기각됐다. 물리 DB 분리 시 이 예외를 재설계한다.
  - 트랜잭션은 인증 도메인 서비스(`AccountRegistrationProcessor`)가 소유하고, 유저 쓰기는 인증 소유 포트(`UserRegistrar` — 앱 어댑터가 유저 도메인 서비스로 구현)로 진입해 참여한다. facade 무트랜잭션·도메인 상호 비의존은 유지된다.
