...
- [x] **6. 본인인증(실) + 약관·동의 정식화 + 내 정보 + 탈퇴·보존 + 휴면** — 실 기관 어댑터 스위치·약관 버전 발행/재동의 게이트·동의 철회 + ConsentState 읽기모델·NotificationPreference 생성·내 정보 조회/수정·탈퇴(PII 즉시 파기·CI tombstone·fail-closed)·재가입 쿨다운(30일)·휴면(12개월·OTP 해제)·`app-batch` 신설(휴면 전환·보존 파기). 완료 메모: NICE 어댑터는 골격+설정 스위치까지 — 실 계약 페이로드 확정·실 호출 검증은 조달(기관 계약·사이트코드) 완료 후 항목. prod 전 법무 형식 최종검토(약관 원문·처리방침)·PIA 필요성 판정이 외부 게이트로 남아 있음(10번). 휴면 사전통지 실 발송은 7번이 마킹을 소비해 배선, 휴면 데이터 물리 분리보관은 후속.
...
- [x] **10. 운영·보안 게이트(prod 배포 전 비협상)** — 부하테스트(핫패스 p99·Lua 경합·Argon2 메모리 예산)·관측성/SLO·컨테이너화/CD(app-migration init·expand-contract)·시크릿 관리(pepper·KEK·서명키·.p8 — 현재 dev 정적 키 평문)·DR/HA(백업/PITR·RTO/RPO·런북)·보안 테스트(STRIDE·펜테스트·SCA·열거 저항)·컴플라이언스(법무·PIA)·OpenAPI 계약. 완료 메모: 코드 갭 8축 전부 해소 — 시크릿 환경변수 주입(fail-fast·dev 부트스트랩), Actuator/Prometheus + 보안 탐지 카운터 + SLO(`docs/ops/observability-slo.md`), 옵트인 부하 하네스(LoadProfileIT) + 실측 전 항목 예산 내(`docs/ops/load-report.md` — 동일 계정 convoy 발견·수용), 공용 Dockerfile 4앱 + migration init 스택 + 태그 릴리스 워크플로 + expand-contract 규약(`docs/ops/deployment.md`), DR 런북(`docs/ops/dr-runbook.md`), Dependabot SCA + STRIDE + 열거 패리티 테스트(`docs/ops/threat-model.md`), springdoc v1 계약. 잔여는 전부 외부 의존 — 법무 최종검토·PIA 판정·펜테스트·조달(본인확인기관·소셜·알림·GeoIP)·prod 인프라(시크릿 매니저·Redis Cluster·PITR·수집기)·prod 유사 재실측: 오너·기한·차단 대상은 `docs/ops/prod-gate.md`가 추적한다.
