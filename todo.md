...
- [ ] **3. 소셜 로그인 4종 + 연동/해제** — 카카오/네이버/구글/애플 OIDC(`external-social` 신설, dev Mock) + 소셜 최초 로그인 온보딩(SOCIAL 스텝셋·CreateUser 재사용) + ≥1 로그인 수단 유지(동시 해제 직렬화) + 애플 특수사항(.p8 동적 client_secret·릴레이 이메일).
