...
/**
 * 외부 호출 없는 Mock 본인인증 어댑터다(dev/test 오프라인 E2E용, {@code user.identity-verification.mode=mock}
 * 기본). 항상 성공을 반환한다.
 *
 * <p>CI/DI는 주장된 정체성(실명·생년월일·휴대폰)에서 <b>결정적으로</b> 파생한다 — 같은 사람의 재시도는
 * 같은 CI를 얻어 CI 원장 유일성 시맨틱이 dev에서도 실기관처럼 동작한다. 생년월일은 ISO-8601
 * ({@code LocalDate#toString()})로 정규화해 파생 입력을 고정한다. 실 기관 어댑터는
 * {@code user.identity-verification.mode=nice}로 전환한다.
 */
@Component
@ConditionalOnProperty(name = "user.identity-verification.mode", havingValue = "mock", matchIfMissing = true)
