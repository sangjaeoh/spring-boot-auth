...
- [x] **P0-6 외부 조달 워크스트림 트래킹 개시** — [`PROCUREMENT.md`](PROCUREMENT.md) 생성(본인확인기관 계약·소셜 4사 앱 심사·Apple Developer·법무 PIPA/PIA — 항목별 오너·상태·리드타임·블로킹 Phase·다음 액션). 오너·리드타임은 미정으로 개시, 조달 실집행 현황은 그 문서에서 트래킹.
...
- [ ] **Phase 1b** — 최소 로컬 온보딩. 기반공사+PII+User·온보딩 플로우 슬라이스 완료·머지, 남은 슬라이스(CreateUser 트랜잭션) 진행.
  - [x] **1b usr 기반공사 + PII 게이트 + User 애그리거트 슬라이스** — `SchemaFlywayFactory`(common-jpa, 스키마별 독립 Flyway·독립 history·`defaultSchema`로 V1 버전충돌 회피) 배선(2번째 스키마 `usr` 등장으로 P0 이연 해제) + app-migration이 auth·usr 멀티스키마 실행(`SchemaMigrationRunner`; Boot Flyway/Hibernate/JpaRepositories auto-config exclude — 단일 Flyway는 `db/migration` 재귀스캔으로 auth V1·usr V1 충돌). §5 PII 게이트: AES-GCM envelope 암호화(`EnvelopeCipher` 포트 common-core / `AesGcmEnvelopeCipher` infra-crypto — KEK가 DEK를 wrap·데이터키 캐싱·암호문에 키버전 동봉→회전 시 구버전 복호로 전 행 재암호화 회피; KMS 포트는 YAGNI 미도입) + 전화 blind index(`BlindIndexer`/`HmacBlindIndexer` — HMAC-SHA256+pepper, pepper 버전 동봉, 보안=pepper 비밀성) + common-jpa PII 컬럼 컨버터(`Encrypted{String,LocalDate}Converter` — Hibernate `SpringBeanContainer`가 `EnvelopeCipher` 빈 주입, 정적홀더 불요). 신규 모듈 `domain-user`(schema usr): `User` 애그리거트(Profile·Contact·PhoneNumber VO 중첩 임베더블, name·birth_date·contact_phone 암호화·contact_email·gender·ci_hash 평문, 전화 blind index 컬럼) + `UserAppender`. 검증: 평문 PII 컬럼 부재(컨버터 출력 정확 일치)·전화 blind index 동등조회·복호 왕복(Testcontainers PG) + 멀티스키마 독립 history(IT). 72 테스트 green. 설계→콜드 설계리뷰(YELLOW)→구현→콜드 코드리뷰(YELLOW, BLOCKING 0)→반영(PhoneNumber E.164 가드·pepper≥32B·키버전 0..255·IT 강화).
  - [x] **1b 온보딩 플로우 슬라이스** — `RegistrationSession`(Redis TTL 30분, 설정 `auth.registration.ttl-minutes`) 애그리거트: domain-auth 포트 `RegistrationSessionStore` + `RegistrationSessionProcessor`(온보딩 전용 토큰=불투명 256bit 랜덤·**해시만 저장**·`MessageDigest.isEqual`·부재 경로 더미 비교로 타이밍 균등화·부재/토큰불일치 단일 404) + infra-redis `RedisRegistrationSessionStore`(단일 hash 키 `reg:<id>` — 생성=단일 Lua HSET+PEXPIRE **원자화**(TTL 없는 고아 키 금지), 마킹=EXISTS 가드 Lua로 **만료 후 좀비 부활 차단**, 전 연산 Redis 장애 503 fail-closed). 이메일/휴대폰 코드는 기존 `VerificationChallenge` 재사용 + `NotificationChannel.SMS` 추가하되 **발급 challengeId를 세션에 종별 바인딩 후 소비 전 대조**(설계리뷰 B-1 — subjectId(=registrationId)만으로는 SMS 코드로 이메일 스텝 위조 가능). Mock 본인인증: 신규 `external-identity` 모듈 + `IdentityVerificationProvider` 포트(domain-user 소유, sealed `Verified`/`Failed`, `provider()`=MOCK enum 값) + `IdentityVerification`(usr V2, 결과 PII 암호화 컬럼·ciHash=`BlindIndexer`(원문 CI 즉시 파생·미저장)·Appender/Modifier/Processor 분리로 기관 호출을 tx 밖) + `CiRegistry`(usr V3, **ci_hash 유니크 DDL 즉시 확보** + 읽기 soft-check `CiUniquenessValidator`) — Mock CI는 정체성(name|ISO생년|phone) 결정 파생이라 dev에서도 동일인=동일 CI. 약관: `TermsDocument`/`TermsVersion`(usr V4) + 필수 3종(SERVICE·PRIVACY_REQUIRED·AGE14) v1 시드(usr V5) + `ConsentValidator`(필수 커버리지·발효버전 조기 검증) + 동의값은 세션에 opaque 문자열 버퍼만(userId 없는 `ConsentRecord` 선-기록 금지). `complete()`=유형별 스텝셋 검증 후 `RegistrationCompletionInfo`(CreateUser 번들: loginEmail·verificationRef·ciHash·consents) 반환 — **엔드포인트·세션 소비는 다음 슬라이스**(멱등키 보존). app-api: 온보딩 7 엔드포인트(permitAll `/auth/registration/**`) + `OnboardingFacade`(PII 제출 전 `requireActive` 선차단) + **멀티스키마 전환 완료**(Boot Flyway exclude → `SchemaMigrationConfig`(SchemaFlywayFactory + EMF dependsOn) — 1b 기반공사 이연 해제) + ERROR 디스패치 permitAll(sendError 400의 401 둔갑 방지). 검증: 스텝 미충족 complete 409·교차 챌린지 거부(챌린지 미소비 보존까지)·TTL 자동 파기+부활 차단·**세션 PII 원문 미보유(Redis 필드셋 정확 일치 단언)**·CI 중복 409·ci_hash 유니크 위반. 99 테스트 green. 설계→콜드 설계리뷰(YELLOW, B-1 반영·M-2/M-3 기각 사유 기록)→구현→콜드 코드리뷰(YELLOW, **BLOCKING 0**, M1 생성 원자화·M2 테스트 격리·M3 @Email 간극 반영)→반영.
  - [ ] **1b CreateUser 단일 크로스스키마 트랜잭션 슬라이스(다음)** — `CreateUser`가 usr(User·ConsentRecord·`CiRegistry.link` 유니크 hard-enforce·IdentityVerification 연결) + auth(AuthAccount·PasswordCredential)를 **한 ACID 트랜잭션(D6 명시 예외 — "한 트랜잭션 하나의 애그리거트"의 문서화된 예외)**에. 트랜잭션 소유자는 도메인 서비스(facade 아님 — facade는 tx 미개방). 중간 실패 전량 롤백(고아-User 불가)·CI 중복 거부·영속 PENDING 금지. `AccountProvisioningFacade` seed 경로를 정식 진입점으로 승격. 입력은 온보딩 `complete()`의 `RegistrationCompletionInfo` + 비밀번호(완료 요청에서 수취) — 완료 엔드포인트 신설, CreateUser 성공 후 세션 소비(멱등키=registrationId), 동의는 CreateUser 트랜잭션에서 현행 약관 재검증 후 append(버퍼 검증은 조기 실패용이었음).
- [ ] **Phase 2** — 소셜 로그인 4종 + 연동/해제
...
### Phase 1b 확정된 구현 결정 (다음 세션 참고)
- 멀티스키마 마이그레이션: `SchemaFlywayFactory`(common-jpa)가 스키마별 독립 Flyway(`schemas(s).defaultSchema(s).locations("classpath:db/migration/"+s)`)를 조립·실행 → 스키마마다 history·버전 독립. 새 스키마 추가는 그 도메인 모듈에 `db/migration/{schema}/` + app-migration `app.migration.schemas`에 등록 + app-migration이 그 도메인을 `runtimeOnly`(리소스). Boot 단일 Flyway는 `db/migration` 재귀스캔으로 스키마 간 V1 충돌하므로 다중스키마 앱은 `FlywayAutoConfiguration`을 exclude한다. app-api는 아직 usr 미영속이라 auth 단일 Boot Flyway 유지 — 다음 슬라이스에서 usr 엔티티를 스캔할 때 app-api도 `SchemaFlywayFactory` + EMF-dependsOn 순서화로 전환(ddl-auto=validate 전 usr 마이그레이션 필요).
- PII crypto: 포트 `EnvelopeCipher`·`BlindIndexer`는 `common-core.crypto`, 구현은 infra-crypto(@Component, `@Value` CSV로 버전→키 맵). PII 컨버터는 common-jpa(`crypto/`). **컨버터는 Spring 빈이 아니어도 됨** — Hibernate `SpringBeanContainer`가 JPA 규약상 `createBean`으로 fresh 조립하며 생성자의 `EnvelopeCipher` 빈을 오토와이어한다(등록한 컨버터 @Bean은 무시됨). 따라서 PII를 영속하는 모든 컨텍스트는 `EnvelopeCipher` 빈 필수(app=infra-crypto, 도메인 IT=fake @Bean). `BlindIndexer`는 도메인 서비스(`UserAppender`)가 평문에서 계산해 엔티티 컬럼에 주입(엔티티는 빈 주입 불가).
- 암호화 컬럼 정본은 DOMAIN_MODEL 영속 유의절이 소유: `users`의 name·birth_date·contact_phone만 암호화(varchar, DATE 아님), contact_email·gender·ci_hash 평문. `ci_hash`는 users에 인덱스만(전역 유니크 hard-enforce는 CiRegistry 소유 — 다음 슬라이스). 전화 blind index 컬럼은 비유니크(같은 전화 다행 가능, 활성 1인은 CiRegistry가 강제)라 리포지토리 조회는 `List` 반환.
- 도메인 VO seam 격리: domain-user는 자체 `Email`/`EmailConverter`/`LifecycleStatus`를 둔다(auth의 동명 타입과 공유 금지). 중첩 @Embeddable record(Contact→PhoneNumber) + 컴포넌트 @Convert/@Enumerated는 Hibernate 7.4에서 동작 확인. PhoneNumber는 E.164 형식 검증(정규화는 upstream 소유).
- CreateUser(다음 슬라이스): usr+auth 크로스스키마를 **한 ACID 트랜잭션**으로(D6 승인 예외 — "한 트랜잭션 하나의 애그리거트"의 명시적 문서화 예외). 트랜잭션 소유자는 도메인 서비스(facade는 tx 미개방). 콜드 리뷰가 이벤트 기반 분리를 제안했으나 D6(단일 PostgreSQL)가 사가·고아를 없애려 채택한 결정이라 단일 ACID 유지가 정답 — 재론 방지.
- 온보딩(플로우 슬라이스 확정, 재론 방지): (1) 챌린지 종별↔스텝 바인딩 — 발급 challengeId를 세션 필드(email/phoneChallengeId)에 보관, verify가 **소비 전** 대조(불일치 제출이 유효 챌린지를 소진시키지 않음), 재발송은 바인딩 교체로 이전 발급분 무효화. 1a `VerificationChallengeStore` 포트는 무변경. (2) ciHash는 세션에 보관(salted HMAC=가명정보, "PII 원문 미보유"의 원문 아님 — 모델 `CreateUser(verificationRef, ciHash, …)` 시그니처 정합). (3) 온보딩 도메인 이벤트(RegistrationStarted 등) 미발행 — 구독자 부재·동기 반환 대체·이벤트 인프라는 todo #4. (4) 챌린지 발급/검증 조율은 facade가 아니라 `RegistrationSessionProcessor` 소유 — 바인딩 불변식이 세션 애그리거트 소유물이라서(PasswordFacade 선례는 RDB 애그리거트 크로스 조율이라 상황 다름). (5) 동의 검증 권위: 버퍼 전 facade→`ConsentValidator`는 조기 실패용, 최종 권위는 CreateUser 재검증(버퍼~완료 사이 약관 개정 경합). (6) `complete()`는 세션을 소비하지 않고 번들만 반환 — CreateUser 성공 후 소비(멱등키 보존). (7) Terms 엔티티는 읽기 전용(시드가 유일 writer — Java 팩토리 없음, P4에서 추가), `Provider` enum에 MOCK 포함(dev 정직 표기). (8) SB4 함정 추가분: `sendError`(역직렬화 400 등)의 ERROR 디스패치가 `anyRequest().authenticated()`에 걸려 401 둔갑 → `dispatcherTypeMatchers(ERROR).permitAll()` 필요. Jakarta `@Email`은 점 없는 도메인을 통과시켜 도메인 `Email` IAE가 500 됨 → DTO에 도메인 동치 `@Pattern`.
- 온보딩 이연·기록(후속 소유 명시): 재발송 쿨다운·일일 발송 한도·레이트리밋(P5), 재가입 쿨다운 판정(P4), IdentityVerification EXPIRED 스윕(P4 — 포트 런타임 예외 시 REQUESTED 잔존은 이 스윕이 수렴, FAILED 오분류 금지), loginEmail 중복 선체크 없음(다음 슬라이스 AuthAccount 유니크가 hard-enforce·열거 저항 설계 병행), SOCIAL 온보딩(P2 — `RegistrationType.SOCIAL` 스텝셋은 선언됨). 콜드 리뷰 기록만: `VerificationChallengeStore.issue`는 Redis 장애 시 raw 500(1a 기존 — 503 정리 후속), 로그인 경로에도 동일 `@Email` 간극 기존재, docs/architecture.md "entity 모듈 밖 시그니처 금지" 문구와 경계-공유 enum(Gender·TermsType 등) 노출의 정합화(ArchUnit은 `@Entity`만 검사) 후속.
