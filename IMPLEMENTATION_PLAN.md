...
- [ ] **Phase 1b** — 최소 로컬 온보딩. 첫 슬라이스(기반공사+PII+User) 완료·머지, 남은 슬라이스(온보딩 플로우 → CreateUser 트랜잭션) 진행.
  ...
  - [ ] **1b 온보딩 플로우 슬라이스(다음)** — `RegistrationSession`(Redis TTL) + 스텝 마킹(이메일/휴대폰=기존 `VerificationChallenge` 재사용) + Mock 본인인증(신규 `external-identity` + `IdentityVerificationProvider` 포트, `IdentityVerification`·`CiRegistry` 링크) + 필수동의 버퍼(`Terms`/`Consent` 시드) + 온보딩 전용 토큰 → 검증: 스텝 미충족 거부.
  - [ ] **1b CreateUser 단일 크로스스키마 트랜잭션 슬라이스** — `CreateUser`가 usr(User·ConsentRecord·`CiRegistry.link` 유니크 hard-enforce·IdentityVerification 연결) + auth(AuthAccount·PasswordCredential)를 **한 ACID 트랜잭션(D6 명시 예외 — "한 트랜잭션 하나의 애그리거트"의 문서화된 예외)**에. 트랜잭션 소유자는 도메인 서비스(facade 아님 — facade는 tx 미개방). 중간 실패 전량 롤백(고아-User 불가)·CI 중복 거부·영속 PENDING 금지. `AccountProvisioningFacade` seed 경로를 정식 진입점으로 승격.
- [ ] **Phase 2** — 소셜 로그인 4종 + 연동/해제
